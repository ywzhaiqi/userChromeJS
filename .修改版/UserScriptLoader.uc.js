...
// @note           modified by Coolkids: 脚本菜单中键根据 downloadURL > updateURL > homepageURL 打开链接，http://bbs.kafan.cn/thread-1750122-1-1.html
...
USL.saveScript = function() {  // 可能存在乱码问题
    ...
    var callbackObj = {
            ...
            // insert downloadURL
            if (!data.match(/\/\/\s*@downloadURL/i)) {
                data = data.replace(/\/\/\s*==\/UserScript==/i,
                    '// @downloadURL  ' + doc.URL + '\n// ==/UserScript==');
            }

            saveFile(fp.file, data);
...
USL.menuClick = function(event){
    ...
    if (event.button == 1) {//edited by dannylee
        //alert(menuitem.script.downloadURL);
        //menuitem.doCommand();
        ...
        if(menuitem.script){
            var url = null;
            if (menuitem.script.downloadURL) {
                url = menuitem.script.downloadURL;
            } else if (menuitem.script.updateURL) {
                url = menuitem.script.updateURL.replace(/\.meta\.js/, ".user.js");
            } else if (menuitem.script.homepageURL) {
                url = menuitem.script.homepageURL;
            }
            //alert(url);
            if (url != null){
                openLinkIn(url,  "tab", {});
...
USL.loadSetting = function() {
    try {
        ...
        aFolder.appendRelativePath('Local');

...
var super_preloader = function(window, document, db) {
    ...
    init(window, document, db);

    //初始化函数..
    function init(window, document, db) {
        ...
        function autopager(SSS, floatWO) {
            ...
            function createSep() {
                ...
                if (SSS.a_separator) {
                    ...
                    a.innerHTML = 'super_preloader自动翻页: 第 <b><span style="color:red!important;">' + curNumber + '</span></b> 页';
        ...
        function prefetcher(SSS, floatWO) {
            ...
            if (SSS.useiframe) {
                ...
                xhttpRequest(nextlink, function(req) {
                    var str = req.responseText;
...
var ns = window.uSuper_preloader = {
    ...
    launch: function(win){
        ...
        if(window.name == "MyNovelReader" || doc.body.getAttribute("name") == 'MyNovelReader')
            return;
        ...
        super_preloader(win, win.document, ns.db);
    },
    load_DB: function(isAlert){
        ...
        if (isAlert)
            Cc['@mozilla.org/alerts-service;1'].getService(Ci.nsIAlertsService)
                .showAlertNotification(null, 'uSuper_preloader.db', '载入成功', false, "", null, "");
        return true;
    }
};

...
window.SimpleDragModY = {
        ...
        switch (event.type) {
            ...
            case "drop":
                if (this.startPoint && event.target.localName != "textarea" && (!(event.target.localName == "input" && (event.target.type == "text" || event.target.type == "password"))) && event.target.contentEditable != "true") {
                    ...
                    this.openUrl(event, where, inBackground);
    ...
    openUrl: function(event, where, inBackground) {
        var transfer = event.dataTransfer;
        var url, searchText;

        if (transfer.types.contains("application/x-moz-file-promise-url")) {
            // 图片
            url = transfer.getData("application/x-moz-file-promise-url");
        } else if (transfer.types.contains("text/x-moz-url")) {
            // 链接
            ...
            } else if (this.localLinkRegExp.test(searchText)) {  // 本地文件
                url = searchText;
